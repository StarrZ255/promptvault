...
export default function Sidebar({ onOpenSettings }: { onOpenSettings: () => void }) {
  const { themes, filter, setFilter, isDark, toggleTheme, refreshThemes, t } = useApp();
  ...
  const handleReorder = useCallback((newOrder: Theme[]) => {
    ...
    const updateDB = async () => {
      await Promise.all(newOrder.map((tItem, i) => window.vault.themes.reorder(tItem.id, i)));
    };
    updateDB();
  }, [refreshThemes]);
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-2 space-y-0.5 overflow-y-auto pb-2">
          ...
          >
            <span>🗄️</span><span>{t('sidebar.all_prompts')}</span>
          </button>
              ...
              >
                {editingId === theme.id ? (
                  <div className="mx-1 my-0.5 p-2 bg-bg border border-primary/40 rounded-xl space-y-1.5">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => saveEdit(theme.id)}
                        className="flex-1 py-1 rounded-lg bg-primary text-white text-xs font-semibold">✓ {t('actions.ok')}</button>
                  ...
                  >
                    ...
                    <span className="flex-1 truncate min-w-0 pr-1 tracking-tight">{theme.label}</span>
        ...
        <div className="p-3 space-y-1 border-t border-border flex-shrink-0">
          <button onClick={toggleTheme}
            className="w-full text-left px-3 py-2 rounded-lg text-sm text-muted hover:text-text hover:bg-white/5 flex items-center gap-2 transition-colors">
            <span>{isDark ? '☀️' : '🌙'}</span>
